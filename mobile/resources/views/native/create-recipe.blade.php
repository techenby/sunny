...
<scroll-view ref="create-recipe-screen" fill class="bg-theme-background">
    <column class="w-full gap-6 px-6 py-6">
        ...
        @if ($error !== '')
            <text ref="create-recipe-error" class="text-sm text-theme-destructive">
                {{ $error }}
            </text>
        @endif

        <button
            ref="create-recipe-submit"
            variant="primary"
            size="lg"
            font="semibold"
            class="w-full"
